	...
	$('a.button.process').click(function(){
		...
		if(body.hasClass('encrypt')){
			...
			reader.onload = function(e){
				...
				const doubleEncrypted = CryptoJS.Rabbit.encrypt(encrypted, password);

				// The download attribute will cause the contents of the href
				// attribute to be downloaded when clicked. The download attribute
				// also holds the name of the file that is offered for download.

				a.attr('href', 'data:application/octet-stream,' + doubleEncrypted);
			...
			reader.onload = function(e){
				...
				const doubleDecrypted = CryptoJS.AES.decrypt(decrypted, password).toString(CryptoJS.enc.Latin1);

				if(!/^data:/.test(doubleDecrypted)){
					alert("Invalid pass phrase or file! Please try again.");
					return false;
				}

				a.attr('href', doubleDecrypted);
